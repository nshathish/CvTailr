    ...
    ILogger<CvParsingService> logger) : ICvParsingService
{
    ...
    private async Task<CvDocument> ExtractDocumentFromLatexAsync(string rawLatexSource, CancellationToken cancellationToken = default)
    {
    ...
    private static bool TryParseDate(string? value, out DateOnly result)
    {
        ...
        if (DateOnly.TryParseExact(value, "yyyy-MM", CultureInfo.InvariantCulture, DateTimeStyles.None, out var yearMonth))
        {
